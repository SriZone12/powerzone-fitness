...
export default function AdminStaff() {
  ...
  const fetchData = async () => {
    const [usersRes, plansRes] = await Promise.all([
      supabase.from('app_users').select('*').eq('role', 'member').order('full_name'),
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-8">
        <div className="bg-gray-900 rounded-lg p-3 mb-6 inline-block">
          <p className="text-gray-400 text-sm">Total Members: <span className="text-white font-bold">{allUsers.length}</span></p>
        </div>
          ...
          <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
            <table className="w-full min-w-[600px]">
              ...
              <tbody>
                  ...
                  <tr key={u.id} className="border-b border-gray-700 hover:bg-gray-800">
                    ...
                    <td className="p-3">
                      <div className="flex gap-2">
                        <button onClick={() => openSetupModal(u)}
                ...
                {allUsers.length === 0 && (
                  <tr><td colSpan="3" className="p-3 text-center text-gray-400">No members yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {showModal && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold text-white mb-4">Member Profile</h3>
            ...
            <button onClick={() => setShowModal(false)}
              className="w-full bg-gray-700 text-white py-3 rounded font-bold hover:bg-gray-600 mt-6">
              Close
            </button>
          </div>
        </div>
      ...
      {showSetupModal && setupMember && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-lg">
            ...
            <p className="text-blue-400 text-sm mb-4">For: <strong>{setupMember.full_name}</strong></p>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSetupMembership}
                className="flex-1 bg-blue-500 text-white py-3 rounded font-bold hover:bg-blue-600">
                Create Membership
              </button>
            </div>
          </div>
